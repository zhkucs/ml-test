...
QString ExtraMeshDecoratePlugin::decorationInfo(FilterIDType filter) const
{
	...
	case DP_SHOW_GRADIENT:		  return tr("Draw per face gradient the current mesh");
	case DP_MEAN_CURVATURE:        return tr("Draw per face curvature gradient on current mesh");
	case DP_HHD_CURLFREE:        return tr("HHD CURLFREE");
	case DP_HHD_DIVFREE:        return tr("HHD DIVFREE");
	case DP_HHD_HARMONEY:        return tr("HHD HARMNEY");
...
QString ExtraMeshDecoratePlugin::decorationName(FilterIDType filter) const
{
	...
	case DP_SHOW_GRADIENT: return QString("Show Gradient");
	case DP_MEAN_CURVATURE:return QString("Show Curvature Gradient");
	case DP_HHD_CURLFREE:        return QString("HHD CURLFREE");
	case DP_HHD_DIVFREE:        return QString("HHD DIVFREE");
	case DP_HHD_HARMONEY:        return QString("HHD HARMNEY");
...
void ExtraMeshDecoratePlugin::decorateMesh(QAction *a, MeshModel &m, RichParameterSet *rm, GLArea *gla, QPainter *painter,GLLogStream &_log)
{
	...
	case DP_SHOW_GRADIENT:
		{
			vcg::Point3f** m_Face_Gradient = new vcg::Point3f*[m.cm.face.size()];
			for(int i = 0 ; i < m.cm.face.size(); i ++){
				m_Face_Gradient[i] = new vcg::Point3f[3];
			}
			glPushAttrib(GL_ENABLE_BIT );
			float NormalLen=rm->getFloat(NormalLength());
			float LineLen = m.cm.bbox.Diag()*NormalLen;
			...
			for(unsigned int i = 0; i<  m.cm.face.size(); i++){
				...
				vcg::Point3f p0 = f.P0(0); 
				vcg::Point3f p1 = f.P0(1); 
				vcg::Point3f p2 = f.P0(2); 
				vcg::Point3f vij=f.P0(1)-f.P0(0);  
				vcg::Point3f vik=f.P0(2)-f.P0(0);
				vcg::Point3f normalf=vij ^ vik;  
				float areaf=0.5f*normalf.Norm();  

				vcg::Point3f nr = normalf.Normalize();
				vcg::Point3f bc = Barycenter(f);
				double r = Distance(p0,bc);
				/*for (int k=0;k<3;k++)  // 分别计算x,y,z的梯度
				{  
				m_Face_Gradient[i][k]=vcg::Point3f(0,0,0);  
				for (int j=0;j<3;j++)  // 三角形走一圈
				{  
				vcg::Point3f ei=f.P0((j+2)%3)-f.P0((j+1)%3);  
				vcg::Point3f gradient=(normalf ^ ei)*(f.P0(j)[k]*0.5f/areaf);  
				m_Face_Gradient[i][k]+=gradient;  
				}    
				} */
				vcg::Point3f axis[] = {vcg::Point3f(1,0,0),vcg::Point3f(0,1,0),vcg::Point3f(0,0,1)};
				for(int k =0; k < 3; k++)
				{
					double norm = 10*(axis[k] * normalf);
					if(abs(norm) < EPSLON){
						norm = 0.5*(norm>=0? 1 : -1);
					}
					m_Face_Gradient[i][k] = ((axis[k] ^ normalf)^normalf);
					double d = Distance(m_Face_Gradient[i][k],vcg::Point3f(0,0,0));
					if(d > r){
						double rr = r /d;
						m_Face_Gradient[i][k] = m_Face_Gradient[i][k]* rr;
					}
					//if(d >10)
				}

				vcg::Point3f start = Barycenter(f);
				vcg::Point3f end =  start + m_Face_Gradient[i][2];//取出z方向的梯度

				//	vcg::Matrix44<float> tr1; tr1.SetTranslate(start);
				vcg::Point3f rotAxis=vcg::Point3f(0,1,0) ^ m_Face_Gradient[i][2];
				rotAxis.Normalize();
				float angleRad = vcg::Angle(vcg::Point3f(0,1,0),m_Face_Gradient[i][2]);
