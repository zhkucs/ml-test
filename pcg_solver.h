...
{
   double sum = 0.;
   std::vector<T>::const_iterator xi = x.begin();
   std::vector<T>::const_iterator yi = y.begin();
   int kk =0;
   while( xi != x.end() )
   {
	   double d1 = (*xi);
...
template<class T>
inline T abs_max( std::vector<T> &x)
// inf-norm (maximum absolute value)
{
   T norm = 0.;

   for( std::vector<T>::const_iterator xi = x.begin(); xi != x.end(); xi++ )
   {
      norm = std::max( norm, *xi );
   }
...
{
   std::vector<T>::const_iterator xi = x.begin();
   std::vector<T>::iterator       yi = y.begin();
...
struct PCGSolver
{
   PCGSolver(void)
   {
      set_solver_parameters(1e-5, 100, 0.97, 0.25,false);
   }

   void set_solver_parameters(T tolerance_factor_, int max_iterations_, T modified_incomplete_cholesky_parameter_=0.97, T min_diagonal_ratio_=0.25, bool use_mic_=false)
   {
      use_mic = use_mic_;
